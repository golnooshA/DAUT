import os
import cv2
import numpy as np
import torch
import torch.nn.functional as F
from torch.autograd import Variable
from torchvision.utils import save_image

# Import your Generator model
from net.Ushape_Trans import Generator  

# Ensure no multiprocessing conflict
os.environ["KMP_DUPLICATE_LIB_OK"] = "TRUE"

def normalize_depth(img_depth):
    """
    Normalize depth map to the range [0, 1].
    """
    img_depth = img_depth.astype(np.float32)
    min_depth = np.min(img_depth)
    max_depth = np.max(img_depth)
    if max_depth != min_depth:
        img_depth = (img_depth - min_depth) / (max_depth - min_depth)
    return img_depth

def postprocess_image(image):
    """
    Adaptive post-processing to balance contrast and saturation.
    """
    # Convert to NumPy array and scale to [0, 255]
    image = image.detach().cpu().numpy().transpose(1, 2, 0)
    image = (image * 255.0).astype(np.uint8)

    # Convert to LAB color space
    lab = cv2.cvtColor(image, cv2.COLOR_RGB2LAB)
    l, a, b = cv2.split(lab)

    # Enhance brightness (CLAHE)
    clahe = cv2.createCLAHE(clipLimit=2.0, tileGridSize=(8, 8))
    l = clahe.apply(l)

    # Merge channels back
    lab = cv2.merge((l, a, b))
    image = cv2.cvtColor(lab, cv2.COLOR_LAB2RGB)

    # Convert back to float and scale to [0, 1]
    image = image.astype(np.float32) / 255.0
    return torch.from_numpy(image).permute(2, 0, 1)

def process_image(image_path, depth_path, device):
    """
    Process input images and depth maps, ensuring proper resizing and normalization.
    """
    img_rgb = cv2.imread(image_path)
    if img_rgb is None:
        raise FileNotFoundError(f"RGB image not found: {image_path}")
    img_rgb = cv2.resize(img_rgb, (256, 256))
    img_rgb = cv2.cvtColor(img_rgb, cv2.COLOR_BGR2RGB) / 255.0

    img_depth = cv2.imread(depth_path, 0)  # Load depth as grayscale
    if img_depth is None:
        raise FileNotFoundError(f"Depth map not found: {depth_path}")
    img_depth = cv2.resize(img_depth, (256, 256))
    img_depth = normalize_depth(img_depth)
    img_depth = img_depth.reshape((256, 256, 1))

    # Combine RGB and depth into one tensor
    img_combined = np.concatenate((img_rgb, img_depth), axis=2)
    img_combined = torch.from_numpy(img_combined.astype(np.float32)).permute(2, 0, 1).unsqueeze(0)

    return Variable(img_combined).to(device)

def load_model(generator_path, device):
    """
    Load the generator model with pretrained weights.
    """
    generator = Generator().to(device)
    state_dict = torch.load(generator_path, map_location=device)
    generator.load_state_dict(state_dict, strict=False)  # Use strict=False to ignore new keys
    generator.eval()
    return generator

def main():
    # Paths to input, depth, and output directories
    path_images = './dataset/UIEB/input'
    path_depth = './DPT/output_monodepth/UIEB_Changed'
    output_path = './test/new_changes'
    generator_path = './save_model/generator_epoch_60.pth'
    
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    os.makedirs(output_path, exist_ok=True)

    # Load the pre-trained generator model
    generator = load_model(generator_path, device)

    # List all image files in the input folder
    image_files = [f for f in os.listdir(path_images) if f.lower().endswith(('.png', '.jpg', '.jpeg'))]

    for img_file in image_files:
        try:
            img_path = os.path.join(path_images, img_file)
            depth_path = os.path.join(path_depth, f"{os.path.splitext(img_file)[0]}depth.png")

            # Process input image and depth map
            input_tensor = process_image(img_path, depth_path, device)

            # Generate enhanced image
            with torch.no_grad():
                output = generator(input_tensor)
                output_tensor = torch.clamp(output[3], 0, 1)  # Use the final output

            # Apply post-processing
            enhanced_img = postprocess_image(output_tensor.squeeze(0))

            # Save the enhanced image
            save_image(enhanced_img, os.path.join(output_path, img_file), nrow=1, normalize=False)
            print(f"Processed {img_file}")

        except Exception as e:
            print(f"Error processing {img_file}: {e}")

if __name__ == "__main__":
    main()
